...
public class BrandApiService(HttpClient httpClient) : IBrandService
{
    ...
    public async Task<Guid> CreateBrandAsync(CreateBrandRequest request)
    {
        var response = await PostAsync("api/brands", request);
        var payload = await response.Content.ReadFromJsonAsync<IdResponse>(JsonOptions);
        return payload?.Id ?? Guid.Empty;
    }

    public async Task UpdateBrandAsync(Guid id, UpdateBrandRequest request)
    {
        await PutAsync($"api/brands/{id}", request);
    }
