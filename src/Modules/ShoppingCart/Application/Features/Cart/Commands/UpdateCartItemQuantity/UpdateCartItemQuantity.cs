...
public class UpdateCartItemQuantity
{
    ...
    public class Handler(IShoppingCartUnitOfWork uow) : IRequestHandler<Command, CartDto>
    {
        public async Task<CartDto> Handle(Command command, CancellationToken cancellationToken)
        {
            ...
            cart.UpdateItemQuantity(command.VariantId, command.Request.Quantity);
    ...
    public class Endpoint : IEndpoint
    {
        ...
        {
            app.MapPut("api/cart/items/{variantId:guid}", async (
                Guid variantId,
                UpdateCartItemQuantityRequest request,
                ISender sender,
                ...
                IHttpContextAccessor httpContextAccessor) =>
            {
                ...
                var result = await sender.Send(new Command(variantId, request, owner.CustomerId, owner.GuestId));
